...
export default function LoginScreen({ onLoginSuccess }) {
  ...
  return (
    <div className="px-4 py-4 space-y-3.5 max-w-md mx-auto min-h-full flex flex-col justify-between bg-zinc-50/40 relative">
      {/* Brand Header */}
      <div className="space-y-1 text-center pt-0.5">
        <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-mono font-bold text-emerald-700">
          <Cpu className="w-3 h-3 text-emerald-600 animate-pulse" />
          <span>ROADVISION AI SECURITY PORTAL</span>
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
        </div>
        
        <h2 className="text-xl sm:text-2xl font-space font-bold text-black tracking-tight">
      ...
      <div className="flex items-center p-1 bg-zinc-200/70 rounded-xl border border-black/5">
        <button
          type="button"
          onClick={() => setAuthMode('login')}
          className={`flex-1 py-1.5 text-[11px] font-space font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
            authMode === 'login'
        ...
        >
          <User className="w-3 h-3" /> Register Account
        </button>
      </div>
          ...
          {roles.map((role) => {
            ...
            return (
              <motion.div
                ...
                className={`relative p-2.5 rounded-xl border transition-all cursor-pointer flex flex-col justify-between space-y-1 ${
                  isSelected
                    ? 'bg-white border-black shadow-sm ring-2 ring-black/10'
                    : 'bg-white/80 border-black/10 hover:bg-white'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className={`w-6 h-6 rounded-lg bg-gradient-to-tr ${role.color} text-white flex items-center justify-center shadow-xs`}>
                ...
                <div>
                  <h3 className="font-space font-bold text-[10px] sm:text-[11px] text-black leading-tight">
                    {role.title}
                  </h3>
                ...
                <div className="flex items-center justify-between border-t border-black/5 pt-0.5">
      ...
      <AnimatePresence mode="wait">
        {authMode === 'login' ? (
          <motion.form
            ...
            className="p-3.5 sm:p-4 rounded-2xl bg-white border border-black/10 shadow-xs space-y-2.5"
          >
            <div className="flex items-center justify-between pb-1.5 border-b border-black/5">
              <span className="text-[11px] font-space font-bold text-black flex items-center gap-1.5">
                <Lock className="w-3 h-3 text-black" /> Credentials Verification
              </span>
              <span className={`text-[8px] font-mono font-bold px-2 py-0.5 rounded text-white bg-gradient-to-r ${activeRoleConfig?.color}`}>
            ...
            <div className="space-y-2">
              ...
              <div>
                <label className="text-[9px] font-mono text-zinc-500 block mb-0.5">Official ID / Email</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-50 border border-black/10 rounded-lg pl-8 pr-3 py-1.5 text-[11px] font-mono text-black focus:outline-none focus:border-black focus:bg-white transition-colors"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-0.5">
                  <label className="text-[9px] font-mono text-zinc-500 block">Security Password</label>
                  <button
                    type="button"
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full bg-zinc-50 border border-black/10 rounded-lg pl-8 pr-8 py-1.5 text-[11px] font-mono text-black focus:outline-none focus:border-black focus:bg-white transition-colors"
                  />
                  <button
                    type="button"
              ...
              <div className="flex items-center justify-between pt-0.5">
                <label className="flex items-center gap-1.5 text-[9px] font-mono text-zinc-600 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={rememberMe}
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-zinc-100 hover:bg-black hover:text-white text-[9px] font-mono text-zinc-700 transition-all cursor-pointer border border-black/5"
                >
            ...
            <motion.button
              ...
              className={`w-full py-2.5 rounded-xl text-white font-space font-bold text-xs tracking-wide shadow-xs flex items-center justify-center gap-2 transition-all cursor-pointer bg-gradient-to-r ${activeRoleConfig?.color}`}
            >
              {isLoading ? (
                ...
                <span className="flex items-center gap-1.5 text-white font-bold">
                  <CheckCircle2 className="w-3.5 h-3.5 text-white" /> Launching Dashboard...
                </span>
              ) : (
                <>
