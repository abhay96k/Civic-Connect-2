...
import { Upload, MapPin, AlertOctagon, CheckCircle2, Sparkles, Send, ShieldCheck } from 'lucide-react';
...
export default function ReportScreen() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const ticket = 'RV-TKT-' + Math.floor(100000 + Math.random() * 900000);
    setTicketId(ticket);
    try {
      ...
      {ticketId ? (
        <div className="py-10 text-center space-y-5 animate-fadeIn">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-600 font-inter mt-1 max-w-xs mx-auto">
              Municipal Repair Crew Alpha has been alerted to dispatch.
            </p>
          </div>
          <button
            onClick={resetForm}
            className="w-full py-3.5 rounded-2xl bg-black text-white font-space font-bold text-xs shadow active:scale-95 transition-all"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="text-center space-y-1">
            ...
            <p className="text-xs text-zinc-500 font-inter">Take or upload a photo for AI laser scanner estimation</p>
          </div>

          {/* Photo Upload Box */}
          <div className="relative min-h-[180px] rounded-3xl border-2 border-dashed border-black/20 bg-zinc-50 flex flex-col items-center justify-center p-4 text-center cursor-pointer overflow-hidden">
            <input
              type="file"
              accept="image/*"
          ...
          {/* Road Address Input */}
          <div>
            <label className="block text-[10px] font-mono text-zinc-700 font-bold mb-1">ROAD ADDRESS</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-zinc-50 border border-black/20 rounded-2xl pl-9 pr-3 py-3 text-xs text-black placeholder-zinc-400 focus:outline-none focus:border-black font-medium"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-zinc-50 border border-black/20 rounded-2xl px-3 py-3 text-xs text-black focus:outline-none focus:border-black font-medium"
            >
          ...
          <button
            type="submit"
            className="w-full py-4 rounded-2xl bg-black text-white font-space font-bold text-xs flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
          >
            <Send className="w-4 h-4" /> Submit Report to Municipal Dispatch
          </button>
        </form>
      )}
    </div>
